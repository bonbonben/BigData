...
result=spark.sql("SELECT plate_id, registration_state, num FROM result WHERE num IN (SELECT MAX(num) as num FROM result)")
result.select(format_string('%s, %s\t%s',result.plate_id,result.registration_state,result.num)).write.save("task5-sql.out",format="text")